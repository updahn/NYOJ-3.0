<template>
  <el-card shadow="always">
    ...
    <vxe-table border="inner" stripe auto-resize align="center" :data="acInfoList">
      <vxe-table-column field="submitTime" min-width="150" :title="$t('m.AC_Time')">
        <template v-slot="{ row }">
          <span>{{ row.submitTime | localtime }}</span>
        </template>
      </vxe-table-column>
      <vxe-table-column field="displayId" :title="$t('m.Problem_ID')" min-width="100">
        <template v-slot="{ row }">
          <span style="vertical-align: top;" v-if="disPlayIdMapColor[row.displayId]">
            <svg
              t="1633685184463"
              class="icon"
      ...
      <vxe-table-column field="first_blood" :title="'AC' + $t('m.Status')" min-width="80">
        <template v-slot="{ row }">
    ...
    <Pagination :total="total" :page-size.sync="limit" :current.sync="page" @on-change="getACInfo"></Pagination>
  </el-card>
</template>
<script>
