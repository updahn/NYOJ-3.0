...
public class BoxFileManager {
    ...
    public Map<Object, Object> uploadFile(MultipartFile file, Long gid)
        ...
        // 若不存在该目录，则创建目录
        FileUtil.mkdir(new File(Constants.File.BOXFILE_BASE_FOLDER.getPath()));

        try {
            // 将文件保存指定目录
            file.transferTo(FileUtil
                    .file(new java.io.File(Constants.File.BOXFILE_BASE_FOLDER.getPath() + File.separator + filename)));
        } catch (Exception e) {
            log.error("文件上传异常-------------->", e);
